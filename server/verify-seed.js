...
async function runChecks(pool, iteration) {
  ...
  assertCondition(Number(summary.ongoingProjects) >= 1, "Expected at least one ongoing project.");
  assertCondition(Number(summary.movedProjects) >= 1, "Expected at least one moved project.");
  assertCondition(Number(summary.cancelledProjects) >= 1, "Expected at least one cancelled project.");
