...
function normalizeProjectPayload(input) {
  ...
  return {
    ...
    beneficiaryTarget: Number.isFinite(input?.beneficiaryTarget) ? Number(input.beneficiaryTarget) : 0,
    dependencies: normalizeDependencies(input?.dependencies),
    publishState: input?.publishState === "published" ? "published" : "draft",
    status: ["upcoming", "ongoing", "moved", "cancelled"].includes(input?.status) ? input.status : "upcoming",
...
async function listProjects({ publishedOnly = false, projectIds = null } = {}) {
  ...
  return projectRows.map((row) => ({
    ...
    status: ["upcoming", "ongoing", "moved", "cancelled"].includes(row.status) ? row.status : "upcoming",
